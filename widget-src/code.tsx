...
function generateTable(table_info:any, x:number, y:number) {
  ...
  for (let i =0; i < table_info.create_definitions.length; i++) {
    ...
    tc.set(col_name, rect);
     ...
     if (DBComponents.isPK(table_info, entry.column.column)) {
      nodes.push(DBComponents.createKeyIcon(x+410, y+55+(i*55)+10, "PK", {r:0.8, g:0.8, b:1.0}));
}
const fk = DBComponents.isFK(table_info, entry.column.column);
if (fk) {
  nodes.push(DBComponents.createKeyIcon(x+410, y+55+(i*55)+10, "FK", {r:1.9, g:1.8, b:1.7}));
...
function Widget() {
  const [tokenID, setTokenID] = useSyncedState("text", "");
  ...
  return (
    <AutoLayout direction="vertical" padding={10}>
    <Input value={tokenID} fontSize={10} width={200} height={30}
      ...
      <Text fontSize={20}
        onClick={(e) => {
          (async () => {
            const url = "https://api.planetscale.com/" + org +"/"+
                      database +
                       "/collate-test-2" ;
                     console.log('this is text', url);
